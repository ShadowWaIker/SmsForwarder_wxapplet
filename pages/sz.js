...
function 设置_被创建(启动参数){
	...
	顶部选项卡1.设置项目(["设备信息","电池详情","远程WOL","设备位置"])
	窗口操作.隐藏面板(0,1)
}
function 重新获取数据(){
    ...
    case 3:
      ...
      对话框1.显示等待框("读取中...")
      网络操作1.发起请求(读写设置.读取设置("服务器")+"/location/query",data,"POST")
	}
}
function 设置_被显示(){
...
function 网络操作1_发起请求完毕(状态码,返回数据){
	...
	if(返回数据.code==200 ){
        ...
        if(返回数据.data.enable_api_location==true ){
					设置列表框1.添加项目("/images/xq.png","远程查定位",false,true,true,"")
					}else{
					设置列表框1.添加项目("/images/xq.png","远程查定位",false,true,false,"")
				}
			break;
			case 1 :
        ...
        对话框1.弹出提示2("发送成功")
        case 3 :
				设置列表框1.添加项目("","地址∶ "+返回数据.data.address,false,false,false,返回数据.data.address,false)
				设置列表框1.添加项目("","经度∶ "+返回数据.data.longitude,false,false,false,返回数据.data.longitude,false)
				设置列表框1.添加项目("","纬度∶ "+返回数据.data.latitude,false,false,false,返回数据.data.latitude,false)
				设置列表框1.添加项目("","供应商∶ "+返回数据.data.provider,false,false,false,返回数据.data.provider,false)
				设置列表框1.添加项目("","上一次定位时间∶ "+返回数据.data.time,false,false,false,返回数据.data.time,false)
			break;
		}
...
optionTap (e) {
  if (设置列表框1.取项目标记(e.detail.项目索引)) {
    wx.setClipboardData({
      data: String(设置列表框1.取项目标记(e.detail.项目索引)),
    })
  }
}
})